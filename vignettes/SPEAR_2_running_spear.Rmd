...
For this vignette, we will load simulated multi-omic data from the "SPEAR_1_simulate_data" vignette: (skip this if using your own data)
...
### Running SPEAR to predict a gaussian (continuous) response:
...
# family - what type of response? Choices include "gaussian", "binomial", "categorical", and "ordinal". See below for how to set up Y for each family
family = "gaussian"
#    Other Parameters:
...
save.name <- "SPEAR_vignette_object_gaussian.rds" # name for SPEARobject (rds file)
seed <- 123 # reproducible seed
```

```{r}
sourceCpp("~/Documents/Coding/IMPACC/SPEAR/packageR6/src/_spear.cpp")
```


```{r}
SPEARobj <- make_spear_object(X = X,
                              Y = Y,
...
options <- c("gaussian", "binomial", "ordinal", "multinomial")
...
Once Y is set up, run SPEAR the same way as above (Gaussian), but change the family parameter (`"binomial"`, `"ordinal"`, `"multinomial"`).
